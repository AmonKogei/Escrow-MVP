...
export default async function runSeed() {
  ...
  try {
    ...
    // create sample escrow if missing
    ...
    if (!existing) {
      await prisma.$transaction(async (tx) => {
        ...
        if (b.balance.lessThan(new Prisma.Decimal(escrowAmount))) {
          throw new Error('Insufficient balance to create escrow during seed.');
        }
        await tx.user.update({ where: { id: buyer.id }, data: { balance: { decrement: escrowAmount } } });
        sampleEscrow = await tx.escrow.create({ data: {
          buyerId: buyer.id,
          sellerId: seller.id,
          amount: escrowAmount,
