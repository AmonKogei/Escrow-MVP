...
export default function BuyerDispute() {
  ...
  const draftKey = 'dispute:buyer:draft';
  ...
  useEffect(() => {
    localStorage.setItem(draftKey, JSON.stringify({ sellerId, amount, description }));
  }, [sellerId, amount, description]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/disputes', {
        ...
        body: JSON.stringify({ buyerId: session.user.id, sellerId, amount, description })
      });
      if (!res.ok) throw new Error(await res.text());
      await res.json();
      toast.success('Dispute created');
      localStorage.removeItem(draftKey);
      setSellerId(''); setAmount(''); setDescription('');
    } catch (err:any) {
  ...
  return (
    <DashboardLayout user={session?.user ?? null} active="dispute">
      <h2 className="text-xl font-semibold mb-4">Buyer Disputes</h2>

      <div className="card bg-white p-4 mb-6">
        <h3 className="font-medium mb-3">Raise Dispute</h3>
        <form onSubmit={submit} className="space-y-3">
          <div>
            <label className="block text-sm font-medium">Seller ID</label>
            <input value={sellerId} onChange={e => setSellerId(e.target.value)} className="input input-bordered w-full" />
          </div>
          <div>
